...
const CategoryPage: React.FC = () => {
  ...
  const handleAddToCart = async (productId: string, quantity: number, price: number) => {
    try {
      await axios.post("/api/add-to-cart", { productId, quantity, price });
  ...
  return (
    <>
      <Header username={session?.user?.name || "Guest"} cartCount={3} onProfileClick={() => {}} />
      <br />
      <br />
      <nav className="categoryNav">
        <ul>
            ...
            return (
              <div key={product._id} className="productItem">
                ...
                <div className="detail">
                  ...
                  <div className="productActions">
                    <button
                      onClick={() => handleAddToCart(product._id, 1, product.discounted_price)}
                      className="addToCartBtn"
                    >
                      <FaCartPlus color="#333" /> Add to Cart
                    </button>
